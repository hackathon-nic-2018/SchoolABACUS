...
<input type="hidden" name="asignatura_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['asignatura_id']); ?>">
<input type="hidden" name="grado_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['grado_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_asignaturas_grados']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_asignaturas_grados"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_asignaturas_grados']['where_detal']))
  {
?>  
...
function lookup_asignatura_id(campo)
{
   dados  = document.F1.asignatura_id.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_asignaturas_grados')?>/form_asignaturas_grados.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_asignatura_id, "asignatura_id", dados, false);
}
function recebe_asignatura_id(string)
{
   document.getElementById('id_lookup_asignatura_id').innerHTML = string;
}
<?php 
