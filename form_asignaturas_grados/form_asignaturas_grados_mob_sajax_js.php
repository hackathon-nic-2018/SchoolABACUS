  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_asignaturas_grados_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function do_ajax_form_asignaturas_grados_mob_validate_asignatura_id()
  {
    var nomeCampo_asignatura_id = "asignatura_id";
    var var_asignatura_id = scAjaxGetFieldText(nomeCampo_asignatura_id);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_asignaturas_grados_mob_validate_asignatura_id(var_asignatura_id, var_script_case_init, do_ajax_form_asignaturas_grados_mob_validate_asignatura_id_cb);
  } // do_ajax_form_asignaturas_grados_mob_validate_asignatura_id

  function do_ajax_form_asignaturas_grados_mob_validate_asignatura_id_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_asignaturas_grados_mob_validate_asignatura_id_cb
  // ---------- Form
  function do_ajax_form_asignaturas_grados_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_asignaturas_grados_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_form_asignaturas_grados_mob_submit_form(var_asignatura_id, var_colegio_id, var_grado_id, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_asignaturas_grados_mob_submit_form_cb);
  } // do_ajax_form_asignaturas_grados_mob_submit_form

  function do_ajax_form_asignaturas_grados_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
  ...
  } // do_ajax_form_asignaturas_grados_mob_submit_form_cb

  var scStatusDetail = {};

  function do_ajax_form_asignaturas_grados_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxProcOn();
    x_ajax_form_asignaturas_grados_mob_navigate_form(var_colegio_id, var_asignatura_id, var_grado_id, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search,  var_nmgp_cond_fast_search,  var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_asignaturas_grados_mob_navigate_form_cb);
  } // do_ajax_form_asignaturas_grados_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_asignaturas_grados_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_asignaturas_grados_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    else if (oResp["navSummary"].reg_tot == 0)
    {
       ...
       sc_hide_form_asignaturas_grados_mob_form();
    }
    sc_mupload_ok = true;
    scAjaxSetFields(false);
    scAjaxSetVariables();
...
{
?>
    do_ajax_form_asignaturas_grados_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
    ...
    SC_btn_grp_text();
  } // do_ajax_form_asignaturas_grados_mob_navigate_form_cb
  function sc_hide_form_asignaturas_grados_mob_form()
  {
  ...
  var ajax_error_list = {
    "asignatura_id": {"label": "Asignatura", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
