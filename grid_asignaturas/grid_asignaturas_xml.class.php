...
class grid_asignaturas_xml
{
   ...
   function monta_xml()
   {
      ...
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['opcao'] = "";
      }
   }
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_asignaturas.xml";
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<grid_asignaturas>\r\n";
         }
         else
         {
             $this->xml_registro = "<grid_asignaturas";
   ...
   function NM_export_asignatura_id()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->asignatura_id))
         {
             $this->asignatura_id = sc_convert_encoding($this->asignatura_id, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <asignatura_id>" . $this->trata_dados($this->asignatura_id) . "</asignatura_id>\r\n";
         }
         else
         {
             $this->xml_registro .= " asignatura_id =\"" . $this->trata_dados($this->asignatura_id) . "\"";
         }
   }
   //----- descripcion
   ...
   function NM_export_especialidad_id()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_especialidad_id))
         {
             $this->look_especialidad_id = sc_convert_encoding($this->look_especialidad_id, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <especialidad_id>" . $this->trata_dados($this->look_especialidad_id) . "</especialidad_id>\r\n";
         }
         else
         {
             $this->xml_registro .= " especialidad_id =\"" . $this->trata_dados($this->look_especialidad_id) . "\"";
         }
   }
   //----- reportar_gobierno
   function NM_export_reportar_gobierno()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_reportar_gobierno))
         {
             $this->look_reportar_gobierno = sc_convert_encoding($this->look_reportar_gobierno, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <reportar_gobierno>" . $this->trata_dados($this->look_reportar_gobierno) . "</reportar_gobierno>\r\n";
         }
         else
         {
             $this->xml_registro .= " reportar_gobierno =\"" . $this->trata_dados($this->look_reportar_gobierno) . "\"";
         }
   }
   //----- colegio_id
   ...
   function NM_export_descripcion_detallada()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <descripcion_detallada>" . $this->trata_dados($this->descripcion_detallada) . "</descripcion_detallada>\r\n";
         }
         else
         {
             $this->xml_registro .= " descripcion_detallada =\"" . $this->trata_dados($this->descripcion_detallada) . "\"";
         }
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_asignaturas']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?> asignaturas :: XML</TITLE>
