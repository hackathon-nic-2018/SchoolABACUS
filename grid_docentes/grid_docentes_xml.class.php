...
class grid_docentes_xml
{
   ...
   function grava_arquivo()
   {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_docentes']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_docentes']['embutida_tit'] . ">\r\n";
         }
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fotografia))
         {
             $this->fotografia = sc_convert_encoding($this->fotografia, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <fotografia>" . $this->trata_dados($this->fotografia) . "</fotografia>\r\n";
         }
         else
         {
             $this->xml_registro .= " fotografia =\"" . $this->trata_dados($this->fotografia) . "\"";
         }
   }
   //----- apellidos
   function NM_export_apellidos()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <apellidos>" . $this->trata_dados($this->apellidos) . "</apellidos>\r\n";
         }
         else
         {
   ...
   function NM_export_nombres()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <nombres>" . $this->trata_dados($this->nombres) . "</nombres>\r\n";
         }
         else
         {
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->identificacion))
         {
             $this->identificacion = sc_convert_encoding($this->identificacion, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <identificacion>" . $this->trata_dados($this->identificacion) . "</identificacion>\r\n";
         }
         else
         {
             $this->xml_registro .= " identificacion =\"" . $this->trata_dados($this->identificacion) . "\"";
         }
   }
   //----- fecha_ingreso
   function NM_export_fecha_ingreso()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fecha_ingreso))
         {
             $this->fecha_ingreso = sc_convert_encoding($this->fecha_ingreso, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <fecha_ingreso>" . $this->trata_dados($this->fecha_ingreso) . "</fecha_ingreso>\r\n";
         }
         else
         {
             $this->xml_registro .= " fecha_ingreso =\"" . $this->trata_dados($this->fecha_ingreso) . "\"";
         }
   }
   //----- especialidad_id
   ...
   function NM_export_docente_id()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->docente_id))
         {
             $this->docente_id = sc_convert_encoding($this->docente_id, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <docente_id>" . $this->trata_dados($this->docente_id) . "</docente_id>\r\n";
         }
         else
         {
             $this->xml_registro .= " docente_id =\"" . $this->trata_dados($this->docente_id) . "\"";
         }
   }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?> docentes :: XML</TITLE>
