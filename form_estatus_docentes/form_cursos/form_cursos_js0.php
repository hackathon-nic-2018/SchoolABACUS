...
<input type="hidden" name="curso_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['curso_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_cursos']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_cursos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_Estudiantes()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Estudiantes(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_grid_curso_estudiantes_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_vglo_colegio*scinvglo_colegio*scout"; ?>', 'inicio', '_blank', '0', '0', 'grid_curso_estudiantes');
}
 NM_tp_critica(1);
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_cursos']['embutida_form']) {
...
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cursos']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_cursos']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cursos']['where_detal']))
  {
?>  
