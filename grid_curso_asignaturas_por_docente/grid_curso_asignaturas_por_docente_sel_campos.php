...
<?php
   ...
   $Sel_Cmp = new grid_curso_asignaturas_por_docente_sel_cmp(); 
   $Sel_Cmp->Sel_cmp_init();
  
class grid_curso_asignaturas_por_docente_sel_cmp
{
function Sel_cmp_init()
{
   ...
   $tab_ger_campos['periodo_id'] = "on";
   $tab_ger_campos['docente_id'] = "on";
   $tab_ger_campos['curso_id'] = "on";
   $tab_ger_campos['asignatura_id'] = "on";
   $tab_ger_campos['colegio_id'] = "on";
   $tab_ger_campos['comentarios'] = "on";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel'] = array();
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['usr_cmp_sel'] = $arr_temp;
?>
    <script language="javascript"> 
<?php
   if (!$this->embbed)
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_othr_grid_title'] ?> curso_asignaturas</TITLE>
...
?>
  function scSubmitSelCampos(sPos) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_curso_asignaturas_por_docente_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['labels'][$NM_cada_field];
           }
       }
   }
...
<?php
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               ...
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsEnabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['labels'][$NM_cada_field]; ?></li>
<?php
               }
               else
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsDisabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_curso_asignaturas_por_docente']['labels'][$NM_cada_field]; ?></li>
<?php
               }
           }
       }
