...
class grid_curso_asignaturas_por_docente_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_curso_asignaturas_por_docente']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_curso_asignaturas_por_docente']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_agrupacion($res_limit=false)
   {
      global $nada, $nm_lang , $periodo_id, $docente_id, $curso_id, $asignatura_id;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['contr_total_geral'] = "OK";
   } 

   //-----  periodo_id
   function quebra_periodo_id_agrupacion($periodo_id, $arg_sum_periodo_id) 
   {
      global $tot_periodo_id, $periodo_id, $docente_id, $curso_id, $asignatura_id;
      $tot_periodo_id = array() ;  
      $tot_periodo_id[0] = $periodo_id ; 
   }
   //-----  docente_id
   function quebra_docente_id_agrupacion($periodo_id, $docente_id, $arg_sum_periodo_id, $arg_sum_docente_id) 
   {
      global $tot_docente_id, $periodo_id, $docente_id, $curso_id, $asignatura_id;
      $tot_docente_id = array() ;  
      $tot_docente_id[0] = $docente_id ; 
   }

   //----- 
   function Calc_resumo_agrupacion($destino_resumo)
   {
      global $nm_lang, $periodo_id, $docente_id, $curso_id, $asignatura_id;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('periodo_id' => "periodo_id",'docente_id' => "docente_id");
      $cmps_quebra_atual = array("periodo_id","docente_id");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_agrupacion($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_agrupacion($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('colegio_id' => 'N','periodo_id' => 'N','curso_id' => 'N','asignatura_id' => 'N','docente_id' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_curso_asignaturas_por_docente']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['docente_id']['reg'] = "S";
      $format_dimensions['docente_id']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'agrupacion', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "periodo_id") {
                  $this->Lookup->lookup_agrupacion_periodo_id($$Cada_dim,  $periodo_id,  $_SESSION['vglo_colegio']);
              }
              if ($Cada_dim == "docente_id") {
                  $this->Lookup->lookup_agrupacion_docente_id($$Cada_dim,  $docente_id,  $_SESSION['vglo_colegio']);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_agrupacion($where_ok);
