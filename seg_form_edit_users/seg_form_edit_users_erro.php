...
$old_error_handler = set_error_handler("seg_form_edit_users_scriptcase_error_handler");

function seg_form_edit_users_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['seg_form_edit_users']['contr_erro']) && $_SESSION['scriptcase']['seg_form_edit_users']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_seg_form_edit_users->contr_seg_form_edit_users->Apl_com_erro = true;
            if ($inicial_seg_form_edit_users->contr_seg_form_edit_users->NM_ajax_flag)
            {
                $inicial_seg_form_edit_users->contr_seg_form_edit_users->NM_ajax_info['errList']['geral_seg_form_edit_users'][] = $msg;
                return;
            }
            seg_form_edit_users_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function seg_form_edit_users_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['seg_form_edit_users'])  && '' != $_SESSION['scriptcase']['error_icon']['seg_form_edit_users'])  ? $_SESSION['scriptcase']['error_icon']['seg_form_edit_users']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['seg_form_edit_users']) && '' != $_SESSION['scriptcase']['error_close']['seg_form_edit_users']) ? $_SESSION['scriptcase']['error_close']['seg_form_edit_users'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['seg_form_edit_users']) && '' != $_SESSION['scriptcase']['error_span_title']['seg_form_edit_users']) ? $_SESSION['scriptcase']['error_span_title']['seg_form_edit_users'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['seg_form_edit_users']) && '' != $_SESSION['scriptcase']['error_icon_title']['seg_form_edit_users']) ? $_SESSION['scriptcase']['error_icon_title']['seg_form_edit_users'] : '';
?>
